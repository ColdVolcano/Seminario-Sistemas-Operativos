...
VBoxManage createhd --filename "$PWD/$NAME/$NAME-vdd.vdi" --size $VDDSIZE --format VDI
VBoxManage storagectl "$NAME" --name "$SATACONNAME" --add sata --controller IntelAhci
VBoxManage storageattach "$NAME" --storagectl "$SATACONNAME" --port 0 --device 0 --type hdd --medium "$PWD/$NAME/$NAME-vdd.vdi"
